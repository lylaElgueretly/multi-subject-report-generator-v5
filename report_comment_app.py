...
step_box(step_col2, 2, "Generate & Select", "Create and choose comments")
step_box(step_col3, 3, "Download", "Export your reports")

st.markdown("<br>", unsafe_allow_html=True)

# Initialize session state for current student's comments
if 'current_comments' not in st.session_state:
    st.session_state.current_comments = {}  # Store both variants for current student
if 'selected_comments' not in st.session_state:
    st.session_state.selected_comments = []  # Store comments selected for download
...
if app_mode == "Single Student":
    ...
    if submitted and name:
        ...
        with st.spinner("Generating both comment variants..."):
            ...
            comment_v2 = generate_comment(subject, year, name, gender, att, achieve,
                                         target, pronouns,
                                         st.session_state.get('attitude_target_input', ''),
                                         variant=2)
            
            # Store both variants in session state
            st.session_state.current_comments = {
                'name': name,
                'subject': subject,
                'year': year,
